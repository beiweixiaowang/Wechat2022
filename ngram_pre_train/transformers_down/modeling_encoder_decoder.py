...
class EncoderDecoderModel(PreTrainedModel):
    r"""
        :class:`~transformers1.EncoderDecoder` is a generic nezha_model class that will be
        instantiated as a transformer architecture with one of the base nezha_model
    ...
    def __init__(
        ...
        ), "The encoder {} should not have a LM Head. Please use a nezha_model without LM Head"
    ...
    def from_encoder_decoder_pretrained(
        ...
        **kwargs
        ...
        r""" Instantiates an encoder and a decoder from one or two base classes of the library from pre-trained nezha_model checkpoints.


        The nezha_model is set in evaluation mode by default using `nezha_model.eval()` (Dropout modules are deactivated).
        To train the nezha_model, you need to first set it back in training mode with `nezha_model.train()`.
        ...
        Params:
            encoder_pretrained_model_name_or_path (:obj: `str`, `optional`, defaults to `None`):
                ...
                - a path to a `directory` containing nezha_model weights saved using :func:`~transformers1.PreTrainedModel.save_pretrained`, e.g.: ``./my_model_directory/encoder``.
            ...
            decoder_pretrained_model_name_or_path (:obj: `str`, `optional`, defaults to `None`):
                ...
                - a string with the `shortcut name` of a pre-trained nezha_model to load from cache or download, e.g.: ``bert-base-uncased``.
                - a string with the `identifier name` of a pre-trained nezha_model that was user-uploaded to our S3, e.g.: ``dbmdz/bert-base-german-cased``.
                - a path to a `directory` containing nezha_model weights saved using :func:`~transformers1.PreTrainedModel.save_pretrained`, e.g.: ``./my_model_directory/decoder``.
                - a path or url to a `tensorflow index checkpoint file` (e.g. `./tf_model/nezha_model.ckpt.index`). In this case, ``from_tf`` should be set to True and a configuration object should be provided as ``config`` argument. This loading path is slower than converting the TensorFlow checkpoint in a PyTorch nezha_model using the provided conversion scripts and loading the PyTorch nezha_model afterwards.
        ...
        Examples::
            ...
            nezha_model = EncoderDecoder.from_encoder_decoder_pretrained('bert-base-uncased', 'bert-base-uncased') # initialize Bert2Bert
        """
        ...
        # The distinction between encoder and decoder at the nezha_model level is made
        # by the value of the flag `is_decoder` that we need to set correctly.
        encoder = kwargs_encoder.pop("nezha_model", None)
        if encoder is None:
            assert (
                encoder_pretrained_model_name_or_path is not None
            ), "If `nezha_model` is not defined as an argument, a `encoder_pretrained_model_name_or_path` has to be defined"
            from .modeling_auto import AutoModel

            encoder = AutoModel.from_pretrained(encoder_pretrained_model_name_or_path, *model_args, **kwargs_encoder)
        encoder.config.is_decoder = False

        decoder = kwargs_decoder.pop("nezha_model", None)
        if decoder is None:
            assert (
            ...
            if "config" not in kwargs_decoder:
                ...
                if decoder_config.is_decoder is False:
                    logger.info(
                        f"Initializing {decoder_pretrained_model_name_or_path} as a decoder nezha_model. Cross attention layers are added to {decoder_pretrained_model_name_or_path} and randomly initialized if {decoder_pretrained_model_name_or_path}'s architecture allows for cross attention layers."
                    )
    ...
    def forward(
        ...
        **kwargs,
        ...
        Examples::
            ...
            nezha_model = EncoderDecoderModel.from_encoder_decoder_pretrained('bert-base-uncased', 'bert-base-uncased') # initialize Bert2Bert
            ...
            # training
            loss, outputs = nezha_model(input_ids=input_ids, decoder_input_ids=input_ids, lm_labels=input_ids)[:2]

            # generation
            generated = nezha_model.generate(input_ids, decoder_start_token_id=nezha_model.config.decoder.pad_token_id)
