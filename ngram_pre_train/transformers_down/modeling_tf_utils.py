...
class TFModelUtilsMixin:
    ...
    def num_parameters(self, only_trainable: bool = False) -> int:
        """
        Get number of (optionally, trainable) parameters in the nezha_model.
        """
        if only_trainable:
...
class TFPreTrainedModel(tf.keras.Model, TFModelUtilsMixin):
    ...
    def resize_token_embeddings(self, new_num_tokens=None):
        ...
        Arguments:
            ...
            new_num_tokens: (`optional`) int:
                ...
                If not provided or None: does nothing and just returns a pointer to the input tokens ``tf.Variable`` Module of the nezha_model.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        r"""Instantiate a pretrained TF 2.0 nezha_model from a pre-trained nezha_model configuration.
        ...
        Parameters:
            pretrained_model_name_or_path: either:
                ...
                - a path or url to a `PyTorch state_dict save file` (e.g. `./pt_model/pytorch_model.bin`). In this case, ``from_pt`` should be set to True and a configuration object should be provided as ``config`` argument. This loading path is slower than converting the PyTorch checkpoint in a TensorFlow nezha_model using the provided conversion scripts and loading the TensorFlow nezha_model afterwards.
            ...
            from_pt: (`optional`) boolean, default False:
                Load the nezha_model weights from a PyTorch state_dict save file (see docstring of pretrained_model_name_or_path argument).
        ...
        Examples::
            ...
            nezha_model = BertModel.from_pretrained('./tf_model/my_tf_checkpoint.ckpt.index', from_pt=True, config=config)
        ...
        try:
            ...
            raise OSError(
                ...
                "If you tried to load a TF 2.0 nezha_model from a PyTorch checkpoint, please set from_pt=True. "
            )
        ...
        if len(missing_keys) > 0:
            logger.info(
                "Layers of {} not initialized from pretrained nezha_model: {}".format(model.__class__.__name__, missing_keys)
            )
        if len(unexpected_keys) > 0:
            logger.info(
                "Layers from pretrained nezha_model not used in {}: {}".format(model.__class__.__name__, unexpected_keys)
            )
        if len(error_msgs) > 0:
            raise RuntimeError(
    ...
    def generate(
        ...
        Parameters:
            ...
            bos_token_id: (`optional`) int
                Beginning of sentence token if no prompt is provided. Default to specicic nezha_model bos_token_id or None if it does not exist.
        ...
        Examples::
            ...
            nezha_model = TFAutoModelWithLMHead.from_pretrained('distilgpt2')    # Download nezha_model and configuration from S3 and cache.
            outputs = nezha_model.generate(max_length=40)  # do greedy decoding
            print('Generated: {}'.format(tokenizer.decode(outputs[0], skip_special_tokens=True)))

            tokenizer = AutoTokenizer.from_pretrained('openai-gpt')   # Initialize tokenizer
            nezha_model = TFAutoModelWithLMHead.from_pretrained('openai-gpt')    # Download nezha_model and configuration from S3 and cache.
            ...
            nezha_model = TFAutoModelWithLMHead.from_pretrained('ctrl')    # Download nezha_model and configuration from S3 and cache.
            ...
            nezha_model = TFAutoModelWithLMHead.from_pretrained('gpt2')    # Download nezha_model and configuration from S3 and cache.
        ...
        if self.get_output_embeddings() is None:
            raise AttributeError(
                ...
                "Please use another nezha_model class (e.g. `TFOpenAIGPTLMHeadModel`, `TFXLNetLMHeadModel`, `TFGPT2LMHeadModel`, `TFCTRLLMHeadModel`, `TFT5ForConditionalGeneration`, `TFTransfoXLLMHeadModel`)"
            )
