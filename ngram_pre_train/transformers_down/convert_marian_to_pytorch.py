...
CONFIG_KEY = "special:nezha_model.yml"
...
# Group Names Logic: change long opus nezha_model names to something shorter, like opus-mt-en-ROMANCE
...
def write_model_card(
    ...
    """Copy the most recent nezha_model's readme section from opus, and add metadata.
    ...
    content = content.split("\n# ")[-1]  # Get the lowest level 1 header in the README -- the most recent nezha_model.
...
def _parse_readme(lns):
    """Get link and metadata from opus nezha_model card equivalent."""
    subres = {}
    for ln in [x.strip() for x in lns]:
        ...
        for k in ["download", "dataset", "models", "nezha_model", "pre-processing"]:
            if ln.startswith(k):
                break
        else:
            continue
        if k in ["dataset", "nezha_model", "pre-processing"]:
            splat = ln.split(":")
            _, v = splat
            subres[k] = v
        elif k == "download":
...
if __name__ == "__main__":
    ...
    # Required parameters
    parser.add_argument("--src", type=str, help="path to marian nezha_model dir", default="en-de")
    parser.add_argument("--dest", type=str, default=None, help="Path to the output PyTorch nezha_model.")
