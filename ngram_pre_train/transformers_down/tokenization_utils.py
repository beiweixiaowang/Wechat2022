...
VERY_LARGE_INTEGER = int(1e30)  # This is used to set the max input length for a nezha_model with infinite size input
...
def truncate_and_pad(
    ...
    if pad_to_max_length and (pad_token and pad_token_id >= 0):
        ...
        logger.warning(
            ...
            "To remove this error, you can add a new pad token and then resize nezha_model embedding:\n"
            "\ttokenizer.pad_token = '<PAD>'\n\tnezha_model.resize_token_embeddings(len(tokenizer))".format(
...
class SpecialTokensMixin:
    """ SpecialTokensMixin is derived by ``PreTrainedTokenizer`` and ``PreTrainedTokenizerFast`` and
        ...
        nezha_model-independant manner and allow to set and update the special tokens.
    """
    ...
    @property
    def cls_token(self):
        """ Classification token (string). E.g. to extract a summary of an input sequence leveraging self-attention along the full depth of the nezha_model. Log an error if used while not having been set. """
    ...
    @property
    def mask_token(self):
        """ Mask token (string). E.g. when training a nezha_model with masked-language modeling. Log an error if used while not having been set. """
    ...
    def cls_token_id(self):
        """ Id of the classification token in the vocabulary. E.g. to extract a summary of an input sequence leveraging self-attention along the full depth of the nezha_model. Log an error if used while not having been set. """
        return self.convert_tokens_to_ids(self.cls_token)

    @property
    def mask_token_id(self):
        """ Id of the mask token in the vocabulary. E.g. when training a nezha_model with masked-language modeling. Log an error if used while not having been set. """
...
class PreTrainedTokenizer(SpecialTokensMixin):
    ...
    Class attributes (overridden by derived classes):
        ...
        - ``vocab_files_names``: a python ``dict`` with, as keys, the ``__init__`` keyword name of each vocabulary file
            required by the nezha_model, and as associated values, the filename for saving the associated file (string).
        - ``pretrained_vocab_files_map``: a python ``dict of dict`` the high-level keys
            being the ``__init__`` keyword name of each vocabulary file required by the nezha_model, the low-level being the
        ...
        - ``max_model_input_sizes``: a python ``dict`` with, as keys, the `short-cut-names` (string) of the pretrained
            models, and as associated values, the maximum length of the sequence inputs of this nezha_model, or None if the
            nezha_model has no maximum input size.
        - ``pretrained_init_configuration``: a python ``dict`` with, as keys, the `short-cut-names` (string) of the
            ...
            ``__init__``method of the tokenizer class for this pretrained nezha_model when loading the tokenizer with the
            ``from_pretrained()`` method.

    Args:
        - ``model_max_length``: (`Optional`) int: the maximum length in number of tokens for the inputs to the transformer nezha_model.
            When the tokenizer is loaded with `from_pretrained`, this will be set to the value stored for the associated
            nezha_model in ``max_model_input_sizes`` (see above). If no value is provided, will default to VERY_LARGE_INTEGER (`int(1e30)`).
            no associated max_length can be found in ``max_model_input_sizes``.
        - ``padding_side``: (`Optional`) string: the side on which the nezha_model should have padding applied.
        ...
        - ``cls_token``: (`Optional`) string: a classification token (e.g. to extract a summary of an input sequence
            leveraging self-attention along the full depth of the nezha_model).
            Will be associated to ``self.cls_token`` and ``self.cls_token_id``
        - ``mask_token``: (`Optional`) string: a masking token (e.g. when training a nezha_model with masked-language
    ...
    NO_PAD_TOKEN_FOR_BATCH_MSG = (
        "No padding token is set for this nezha_model, therefore no batch can be made with uneven "
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        if pretrained_model_name_or_path in s3_models:
            ...
            logger.info(
                "Model name '{}' not found in nezha_model shortcut name list ({}). "
                "Assuming '{}' is a path, a nezha_model identifier, or url to a directory containing tokenizer files.".format(
            ...
            if os.path.isfile(pretrained_model_name_or_path) or is_remote_url(pretrained_model_name_or_path):
                if len(cls.vocab_files_names) > 1:
                    raise ValueError(
                        ...
                        "Use a nezha_model identifier or the path to a directory instead."
                    )
                logger.warning(
                ...
                # At this point pretrained_model_name_or_path is either a directory or a nezha_model identifier name
        ...
        try:
            ...
            if pretrained_model_name_or_path in s3_models:
                ...
                msg = (
                    "Model name '{}' was not found in tokenizers nezha_model name list ({}). "
        ...
        if all(full_file_name is None for full_file_name in resolved_vocab_files.values()):
            raise EnvironmentError(
                "Model name '{}' was not found in tokenizers nezha_model name list ({}). "
                "We assumed '{}' was a path, a nezha_model identifier, or url to a directory containing vocabulary files "
                "named {} but couldn't find such vocabulary files at this path or url.".format(
                    pretrained_model_name_or_path,
                    ", ".join(s3_models),
                    pretrained_model_name_or_path,
        ...
        if pretrained_model_name_or_path in cls.max_model_input_sizes:
            # if we're using a pretrained nezha_model, ensure the tokenizer
    ...
    def add_tokens(self, new_tokens: Union[str, List[str]]) -> int:
        ...
        Examples::
            ...
            # Let's see how to increase the vocabulary of Bert nezha_model and tokenizer
            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            nezha_model = BertModel.from_pretrained('bert-base-uncased')

            num_added_toks = tokenizer.add_tokens(['new_tok1', 'my_new-tok2'])
            print('We have added', num_added_toks, 'tokens')
            nezha_model.resize_token_embeddings(len(tokenizer))  # Notice: resize_token_embeddings expect to receive the full size of the new vocabulary, i.e. the length of the tokenizer.
        """
        if not new_tokens:
            return 0
    ...
    def add_special_tokens(self, special_tokens_dict):
        ...
        - you can easily refer to special tokens using tokenizer class attributes like `tokenizer.cls_token`. This makes it easy to develop nezha_model-agnostic training and fine-tuning scripts.
        ...
        Examples::
            ...
            nezha_model = GPT2Model.from_pretrained('gpt2')
    ...
    def tokenize(self, text: TextInput, **kwargs):
        """ Converts a string in a sequence of tokens (string), using the tokenizer.
            ...
            Args:
                ...
                **kwargs (:obj: `dict`): Arguments passed to the nezha_model-specific `prepare_for_tokenization` preprocessing method.
        """
        ...
        **kwargs
        ...
        Args:
            ...
            add_special_tokens (:obj:`bool`, `optional`, defaults to :obj:`True`):
                ...
                to their nezha_model.
            max_length (:obj:`int`, `optional`, defaults to :obj:`None`):
                If set to a number, will limit the total sequence returned so that it has a maximum length.
                If there are overflowing tokens, those will be added to the returned dictionary.
                You can set it to the maximal input size of the nezha_model with `max_length = tokenizer.model_max_length`.
            ...
            pad_to_max_length (:obj:`bool`, `optional`, defaults to :obj:`False`):
                If set to True, the returned sequences will be padded according to the nezha_model's padding side and
                padding index, up to their max length. If no max length is specified, the padding is done up to the
                nezha_model's max length. The tokenizer padding sides are handled by the class attribute `padding_side`
    ...
    def encode_plus(
        ...
        **kwargs
        ...
        Return:
            ...
            - ``input_ids``: list of token ids to be fed to a nezha_model
            - ``token_type_ids``: list of token type ids to be fed to a nezha_model
            - ``attention_mask``: list of indices specifying which tokens should be attended to by the nezha_model
    ...
    def batch_encode_plus(
        ...
        **kwargs
        ...
        for first_ids, second_ids in input_ids:
            ...
            # the nezha_model. It adds special tokens, truncates sequences if overflowing while taking into account
    ...
    def prepare_for_model(
        ...
        """ Prepares a sequence of input id, or a pair of sequences of inputs ids so that it can be used by the nezha_model.
        ...
        Args:
            ...
            truncation_strategy: string selected in the following options:
                ...
                - 'only_second': Only truncate the second sequence
                - 'do_not_truncate': Does not truncate (raise an error if the input sequence is longer than max_length)
            pad_to_max_length: if set to True, the returned sequences will be padded according to the nezha_model's padding side and
                padding index, up to their max length. If no max length is specified, the padding is done up to the nezha_model's max length.
            ...
            return_token_type_ids: (optional) Set to False to avoid returning token_type_ids (default: set to nezha_model specifics).
            return_attention_mask: (optional) Set to False to avoid returning attention mask (default: set to nezha_model specifics)
        ...
        # Load from nezha_model defaults
        ...
        if max_length is None and len(encoded_inputs["input_ids"]) > self.model_max_length:
            logger.warning(
                ...
                "for this nezha_model ({} > {}). Running this sequence through the nezha_model will result in "
        ...
        if pad_to_max_length and max_length is None and self.model_max_length > LARGE_INTEGER:
            logger.warning(
                "Sequence can't be padded as no maximum length is specified and the nezha_model maximum length is too high."
            )
        ...
        # Prepare nezha_model inputs as tensors if asked
        if return_tensors == "tf" and is_tf_available():
    ...
    def build_inputs_with_special_tokens(self, token_ids_0: List, token_ids_1: Optional[List] = None) -> List:
        """
        Build nezha_model inputs from a sequence or a pair of sequence for sequence classification tasks
    ...
    def get_special_tokens_mask(
        ...
        Args:
            ...
            already_has_special_tokens: (default False) Set to True if the token list is already formated with
                special tokens for the nezha_model
...
class PreTrainedTokenizerFast(PreTrainedTokenizer):
    ...
    def add_special_tokens(self, special_tokens_dict: dict) -> int:
        ...
        #    - they will never be processed by the nezha_model, and
