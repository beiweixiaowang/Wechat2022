...
class TFT5Attention(tf.keras.layers.Layer):
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        sequences than the nezha_model has been trained on.
        Args:
...
####################################################
# The full nezha_model without a specific pretrained or finetuning head is
...
class TFT5MainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if inputs_embeds is None:
            assert self.embed_tokens is not None, "You have to intialize the nezha_model with valid token embeddings"
        ...
        if num_dims_attention_mask == 3:
            ...
            # - if the nezha_model is an encoder, make the mask broadcastable to [batch_size, num_heads, mask_seq_length, mask_seq_length]
            if self.is_decoder:
...
T5_START_DOCSTRING = r"""    The T5 nezha_model was proposed in
    `Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer`_
    by Colin Raffel, Noam Shazeer, Adam Roberts, Katherine Lee, Sharan Narang, Michael Matena, Yanqi Zhou, Wei Li, Peter J. Liu.
    It's an encoder decoder transformer pre-trained in a text-to-text denoising generative setting.

    This nezha_model is a tf.keras.Model `tf.keras.Model`_ sub-class. Use it as a regular TF 2.0 Keras Model and
    ...
    Note on the nezha_model inputs:
        TF 2.0 models accepts two formats as inputs:

            - having all inputs as keyword arguments (like PyTorch models), or
            - having all inputs as a list, tuple or dict in the first positional arguments.

        This second option is usefull when using `tf.keras.Model.fit()` method which currently requires having all the tensors in the first argument of the nezha_model call function: `nezha_model(inputs)`.

        If you choose this second option, there are three possibilities you can use to gather all the input Tensors in the first positional argument :

        - a single Tensor with inputs only and nothing else: `nezha_model(inputs_ids)
        - a list of varying length with one or several input Tensors IN THE ORDER given in the docstring:
            `nezha_model([inputs, attention_mask])` or `nezha_model([inputs, attention_mask, token_type_ids])`
        - a dictionary with one or several input Tensors associaed to the input names given in the docstring:
            `nezha_model({'inputs': inputs, 'token_type_ids': token_type_ids})`

    Parameters:
        config (:class:`~transformers1.T5Config`): Model configuration class with all the parameters of the nezha_model.
...
T5_INPUTS_DOCSTRING = r"""
    Args:
        ...
        inputs (:obj:`tf.Tensor` of shape :obj:`(batch_size, sequence_length)`):
            ...
            T5 is a nezha_model with relative position embeddings so you should be able to pad the inputs on
        ...
        decoder_past_key_value_states (:obj:`tuple(tuple(tf.Tensor))` of length :obj:`config.n_layers` with each tuple having 4 tensors of shape :obj:`(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
            ...
            (those that don't have their past key value states given to this nezha_model) of shape :obj:`(batch_size, 1)`
...
class TFT5Model(TFT5PreTrainedModel):
    ...
    Return:
        ...
        decoder_past_key_value_states (:obj:`tuple(tuple(tf.Tensor))` of length :obj:`config.n_layers` with each tuple having 4 tensors of shape :obj:`(batch_size, num_heads, sequence_length, embed_size_per_head)`, `optional`, returned when ``use_cache=True``):
            ...
            Note that when using `decoder_past_key_value_states`, the nezha_model only outputs the last `hidden-state` of the sequence of shape :obj:`(batch_size, 1, config.vocab_size)`.
    ...
    Examples::
        ...
        nezha_model = TFT5Model.from_pretrained('t5-small')
        inputs = tokenizer.encode("Hello, my dog is cute", return_tensors="tf")  # Batch size 1
        outputs = nezha_model(inputs, decoder_input_ids=inputs)
...
class TFT5ForConditionalGeneration(TFT5PreTrainedModel):
    ...
    Return:
        ...
        decoder_past_key_value_states (:obj:`tuple(tuple(tf.Tensor))` of length :obj:`config.n_layers` with each tuple having 4 tensors of shape :obj:`(batch_size, num_heads, sequence_length, embed_size_per_head)`, `optional`, returned when ``use_cache=True``):
            Contains pre-computed key and value hidden-states of the attention blocks.
            Can be used to speed up sequential decoding (see `decoder_past_key_value_states` input).
            Note that when using `decoder_past_key_value_states`, the nezha_model only outputs the last `prediction_score` of the sequence of shape :obj:`(batch_size, 1, config.vocab_size)`.
        hidden_states (:obj:`tuple(tf.Tensor)`, `optional`, returned when ``config.output_hidden_states=True``):
            Tuple of :obj:`tf.Tensor` (one for the output of the embeddings + one for the output of each layer)
            of shape :obj:`(batch_size, sequence_length, hidden_size)`.
    ...
    Examples::
        ...
        tokenizer = T5Tokenizer.from_pretrained('t5-small')
        nezha_model = TFT5ForConditionalGeneration.from_pretrained('t5-small')
        inputs = tokenizer.encode("Hello, my dog is cute", return_tensors="tf")  # Batch size 1
        outputs = nezha_model(inputs, decoder_input_ids=inputs)
