...
class OnnxConverterArgumentParser(ArgumentParser):
    ...
    def __init__(self):
        ...
        self.add_argument("--nezha_model", type=str, required=True, help="Model's id or path (ex: bert-base-cased)")
        self.add_argument("--tokenizer", type=str, help="Tokenizer's id or path (ex: bert-base-cased)")
        self.add_argument("--framework", type=str, choices=["pt", "tf"], help="Framework for loading the nezha_model")
        self.add_argument("--opset", type=int, default=11, help="ONNX opset to use")
        self.add_argument("--check-loading", action="store_true", help="Check ONNX is able to load the nezha_model")
        self.add_argument("--use-external-format", action="store_true", help="Allow exporting nezha_model >= than 2Gb")
...
def ensure_valid_input(model, tokens, input_names):
    ...
    Args:
        model: The nezha_model used to forward the input data
...
def load_graph_from_args(framework: str, model: str, tokenizer: Optional[str] = None) -> Pipeline:
    ...
    print("Loading pipeline (nezha_model: {}, tokenizer: {})".format(model, tokenizer))

    # Allocate tokenizer and nezha_model
...
def convert_tensorflow(nlp: Pipeline, opset: int, output: str):
    ...
    print("/!\\ Please note TensorFlow doesn't support exporting nezha_model > 2Gb /!\\")
    ...
    try:
        ...
        raise Exception(
            "Cannot import {} required to convert TF nezha_model to ONNX. Please install {} first.".format(e.name, e.name)
        )
...
def verify(path: str):
    ...
    print("Checking ONNX nezha_model loading from: {}".format(path))
    try:
        onnx_options = SessionOptions()
        _ = InferenceSession(path, onnx_options, providers=["CPUExecutionProvider"])
        print("Model correctly loaded")
    except RuntimeException as re:
        print("Error while loading the nezha_model: {}".format(re))
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Error while converting the nezha_model: {}".format(e))
        exit(1)
