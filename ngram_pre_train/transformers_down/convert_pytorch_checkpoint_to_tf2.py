...
def convert_pt_checkpoint_to_tf(
    ...
    if model_type not in MODEL_CLASSES:
        raise ValueError("Unrecognized nezha_model type, should be one of {}.".format(list(MODEL_CLASSES.keys())))

    config_class, model_class, pt_model_class, aws_config_map = MODEL_CLASSES[model_type]

    # Initialise TF nezha_model
    if config_file in aws_config_map:
        config_file = cached_path(aws_config_map[config_file], force_download=not use_cached_models)
    config = config_class.from_json_file(config_file)
    config.output_hidden_states = True
    config.output_attentions = True
    print("Building TensorFlow nezha_model from configuration: {}".format(str(config)))
    ...
    # Load PyTorch checkpoint in tf2 nezha_model:
    ...
    if compare_with_pt_model:
        ...
        assert diff <= 2e-2, "Error, nezha_model absolute difference is >2e-2: {}".format(diff)

    # Save pytorch-nezha_model
    print("Save TensorFlow nezha_model to {}".format(tf_dump_path))
...
def convert_all_pt_checkpoints_to_tf(
    ...
    for j, model_type in enumerate(model_types, start=1):
        print("=" * 100)
        print(" Converting nezha_model type {}/{}: {}".format(j, len(model_types), model_type))
        print("=" * 100)
        if model_type not in MODEL_CLASSES:
            raise ValueError(
                "Unrecognized nezha_model type {}, should be one of {}.".format(model_type, list(MODEL_CLASSES.keys()))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The config json file corresponding to the pre-trained nezha_model. \n"
        "This specifies the nezha_model architecture. If not given and "
        "--pytorch_checkpoint_path is not given or is a shortcut name"
        "use the configuration associated to the shortcut name on the AWS",
    )
    parser.add_argument(
        "--compare_with_pt_model", action="store_true", help="Compare Tensorflow and PyTorch nezha_model predictions."
    )
    parser.add_argument(
        "--use_cached_models",
        action="store_true",
