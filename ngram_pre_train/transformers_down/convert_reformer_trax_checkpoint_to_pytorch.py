...
def set_model_weights_in_torch(weights, torch_model, hidden_size):
    ...
    assert len(torch_model_reformer.encoder.layers) * 4 == len(
        trax_layer_weights
    ), "HF and trax nezha_model do not have the same number of layers"
...
def convert_trax_checkpoint_to_pytorch(trax_model_pkl_path, config_file, pytorch_dump_path):
    ...
    print("Building PyTorch nezha_model from configuration: {}".format(str(config)))
    ...
    print("Save PyTorch nezha_model to {}".format(pytorch_dump_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The config json file corresponding to the pre-trained Reformer nezha_model. \n"
        "This specifies the nezha_model architecture.",
    )
    parser.add_argument(
        "--pytorch_dump_path", default=None, type=str, required=True, help="Path to the output PyTorch nezha_model."
    )
