...
def get_framework(model=None):
    """ Select framework (TensorFlow/PyTorch) to use.
        If both frameworks are installed and no specific nezha_model is provided, defaults to using PyTorch.
    """
    if is_tf_available() and is_torch_available() and model is not None and not isinstance(model, str):
        # Both framework are available but the user supplied a nezha_model class instance.
        # Try to guess which framework to use from the nezha_model classname
...
class Pipeline(_ScikitCompat):
    ...
    Arguments:
        model (:obj:`~transformers1.PreTrainedModel` or :obj:`~transformers1.TFPreTrainedModel`):
            The nezha_model that will be used by the pipeline to make predictions. This needs to be a nezha_model inheriting from
        ...
        tokenizer (:obj:`~transformers1.PreTrainedTokenizer`):
            The tokenizer that will be used by the pipeline to encode data for the nezha_model. This object inherits from
            :class:`~transformers1.PreTrainedTokenizer`.
        modelcard (:obj:`str` or :class:`~transformers1.ModelCard`, `optional`, defaults to :obj:`None`):
            Model card attributed to the nezha_model for this pipeline.
        ...
        device (:obj:`int`, `optional`, defaults to :obj:`-1`):
            Device ordinal for CPU/GPU supports. Setting this to -1 will leverage CPU, >=0 will run the nezha_model
    ...
    def save_pretrained(self, save_directory):
        """
        Save the pipeline's nezha_model and tokenizer to the specified save_directory
        """
    ...
    def _forward(self, inputs, return_tensors=False):
        ...
        Args:
            inputs: dict holding all the keyworded arguments for required by the nezha_model forward method.
...
class TextGenerationPipeline(Pipeline):
    ...
    def __call__(
        ...
        if self.model.__class__.__name__ not in self.ALLOWED_MODELS:
            raise NotImplementedError(
                "Generation is currently not supported for {}. Please select a nezha_model from {} for generation.".format(
        ...
        for prompt_text in text_inputs:
            ...
            for generated_sequence in output_sequences:
                ...
                if return_text:
                    ...
                    # Remove PADDING prompt of the sequence if XLNet or Transfo-XL nezha_model is used
...
class TextClassificationPipeline(Pipeline):
    ...
    Arguments:
        nezha_model (:obj:`~transformers1.PreTrainedModel` or :obj:`~transformers1.TFPreTrainedModel`):
            The nezha_model that will be used by the pipeline to make predictions. This needs to be a nezha_model inheriting from
            :class:`~transformers1.PreTrainedModel` for PyTorch and :class:`~transformers1.TFPreTrainedModel` for
            TensorFlow.
        tokenizer (:obj:`~transformers1.PreTrainedTokenizer`):
...
class QuestionAnsweringPipeline(Pipeline):
    ...
    def __call__(self, *args, **kwargs):
        ...
        Returns:
            ...
            score: the score the current answer scored for the nezha_model
    ...
    def decode(self, start: np.ndarray, end: np.ndarray, topk: int, max_answer_len: int) -> Tuple:
        ...
        Args:
            ...
            topk: int, indicates how many possible answer span(s) to extract from the nezha_model's output
            max_answer_len: int, maximum size of the answer to extract from the nezha_model's output
        """
...
class SummarizationPipeline(Pipeline):
    ...
    Usage::
        ...
        # use t5 in tf
        summarizer = pipeline("summarization", nezha_model="t5-base", tokenizer="t5-base", framework="tf")
        summarizer("Sam Shleifer writes the best docstring examples in the whole world.", min_length=5, max_length=20)
    ...
    Arguments:
        nezha_model (:obj:`str` or :obj:`~transformers1.PreTrainedModel` or :obj:`~transformers1.TFPreTrainedModel`, `optional`, defaults to :obj:`None`):
            The nezha_model that will be used by the pipeline to make predictions. This can be :obj:`None`, a string
            checkpoint identifier or an actual pre-trained nezha_model inheriting from
            :class:`~transformers1.PreTrainedModel` for PyTorch and :class:`~transformers1.TFPreTrainedModel` for
            TensorFlow.
        ...
        tokenizer (:obj:`str` or :obj:`~transformers1.PreTrainedTokenizer`, `optional`, defaults to :obj:`None`):
            The tokenizer that will be used by the pipeline to encode data for the nezha_model. This can be :obj:`None`,
    ...
    def __call__(
        ...
        Args:
            ...
            **generate_kwargs: extra kwargs passed to `self.nezha_model.generate`_

        Returns:
            list of dicts with 'summary_text' and/or 'summary_token_ids' for each document_to_summarize

        .. _`self.nezha_model.generate`:
...
SUPPORTED_TASKS = {
    "feature-extraction": {
        ...
        "default": {
            "nezha_model": {"pt": "distilbert-base-cased", "tf": "distilbert-base-cased"},
            "config": None,
    ...
    "question-answering": {
        ...
        "default": {
            "nezha_model": {"pt": "distilbert-base-cased-distilled-squad", "tf": "distilbert-base-cased-distilled-squad"},
            "config": None,
    ...
    "fill-mask": {
        ...
        "default": {
            "nezha_model": {"pt": "distilroberta-base", "tf": "distilroberta-base"},
            "config": None,
    ...
    "summarization": {
        ...
        "tf": TFAutoModelWithLMHead if is_tf_available() else None,
        "pt": AutoModelWithLMHead if is_torch_available() else None,
        "default": {"nezha_model": {"pt": "facebook/bart-large-cnn", "tf": "t5-small"}, "config": None, "tokenizer": None},
    },
    "translation_en_to_fr": {
        ...
        "pt": AutoModelWithLMHead if is_torch_available() else None,
        "default": {
            "nezha_model": {"pt": "t5-base", "tf": "t5-base"},
            "config": None,
    ...
    "text-generation": {
        ...
        "tf": TFAutoModelWithLMHead if is_tf_available() else None,
        "pt": AutoModelWithLMHead if is_torch_available() else None,
        "default": {"nezha_model": {"pt": "gpt2", "tf": "gpt2"}, "config": None, "tokenizer": "gpt2"},
    },
}
...
def pipeline(
    ...
    **kwargs
    ...
    Pipeline are made of:
        ...
        - A Model instance
        - Some (optional) post processing for enhancing nezha_model's output
    ...
    Args:
        ...
        model (:obj:`str` or :obj:`~transformers1.PreTrainedModel` or :obj:`~transformers1.TFPreTrainedModel`, `optional`, defaults to :obj:`None`):
            The nezha_model that will be used by the pipeline to make predictions. This can be :obj:`None`,
            a nezha_model identifier or an actual pre-trained nezha_model inheriting from
            :class:`~transformers1.PreTrainedModel` for PyTorch and :class:`~transformers1.TFPreTrainedModel` for
            TensorFlow.
        ...
        config (:obj:`str` or :obj:`~transformers1.PretrainedConfig`, `optional`, defaults to :obj:`None`):
            The configuration that will be used by the pipeline to instantiate the nezha_model. This can be :obj:`None`,
            a nezha_model identifier or an actual pre-trained nezha_model configuration inheriting from
            :class:`~transformers1.PretrainedConfig`.

            If :obj:`None`, the default for this pipeline will be loaded.
        tokenizer (:obj:`str` or :obj:`~transformers1.PreTrainedTokenizer`, `optional`, defaults to :obj:`None`):
            The tokenizer that will be used by the pipeline to encode data for the nezha_model. This can be :obj:`None`,
            a nezha_model identifier or an actual pre-trained tokenizer inheriting from
            :class:`~transformers1.PreTrainedTokenizer`.
    ...
    Examples::
        ...
        pipeline('question-answering', nezha_model='distilbert-base-cased-distilled-squad', tokenizer='bert-base-cased')

        # Named entity recognition pipeline, passing in a specific nezha_model and tokenizer
        nezha_model = AutoModelForTokenClassification.from_pretrained("dbmdz/bert-large-cased-finetuned-conll03-english")
        tokenizer = AutoTokenizer.from_pretrained("bert-base-cased")
        pipeline('ner', nezha_model=nezha_model, tokenizer=tokenizer)
    """
    # Retrieve the task
    ...
    # Use default nezha_model/config/tokenizer for the task if no nezha_model is provided
    if model is None:
        models, config, tokenizer = [targeted_task["default"][k] for k in ["nezha_model", "config", "tokenizer"]]
        model = models[framework]

    # Try to infer tokenizer from nezha_model or config name (if provided as str)
    ...
    modelcard = None
    # Try to infer modelcard from nezha_model or config name (if provided as str)
    ...
    # Instantiate nezha_model if needed
    if isinstance(model, str):
        # Handle transparent TF/PT nezha_model conversion
        model_kwargs = {}
        if framework == "pt" and model.endswith(".h5"):
            model_kwargs["from_tf"] = True
            logger.warning(
                "Model might be a TensorFlow nezha_model (ending with `.h5`) but TensorFlow is not available. "
                "Trying to load the nezha_model with PyTorch."
            )
        elif framework == "tf" and model.endswith(".bin"):
            model_kwargs["from_pt"] = True
            logger.warning(
                "Model might be a PyTorch nezha_model (ending with `.bin`) but PyTorch is not available. "
                "Trying to load the nezha_model with Tensorflow."
            )
