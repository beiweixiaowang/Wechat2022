...
class AutoTokenizer:
    r""":class:`~transformers1.AutoTokenizer` is a generic tokenizer class
        ...
        falling back to using pattern matching on the `pretrained_model_name_or_path` string:
            - `t5`: T5Tokenizer (T5 nezha_model)
            - `distilbert`: DistilBertTokenizer (DistilBert nezha_model)
            - `albert`: AlbertTokenizer (ALBERT nezha_model)
            - `camembert`: CamembertTokenizer (CamemBERT nezha_model)
            - `xlm-roberta`: XLMRobertaTokenizer (XLM-RoBERTa nezha_model)
            - `longformer`: LongformerTokenizer (AllenAI Longformer nezha_model)
            - `roberta`: RobertaTokenizer (RoBERTa nezha_model)
            - `bert`: BertTokenizer (Bert nezha_model)
            - `openai-gpt`: OpenAIGPTTokenizer (OpenAI GPT nezha_model)
            - `gpt2`: GPT2Tokenizer (OpenAI GPT-2 nezha_model)
            - `transfo-xl`: TransfoXLTokenizer (Transformer-XL nezha_model)
            - `xlnet`: XLNetTokenizer (XLNet nezha_model)
            - `xlm`: XLMTokenizer (XLM nezha_model)
            - `ctrl`: CTRLTokenizer (Salesforce CTRL nezha_model)
            - `electra`: ElectraTokenizer (Google ELECTRA nezha_model)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        from a pre-trained nezha_model vocabulary.
        ...
        falling back to using pattern matching on the `pretrained_model_name_or_path` string:
            ...
            - `longformer`: LongformerTokenizer (AllenAI Longformer nezha_model)
            - `roberta`: RobertaTokenizer (RoBERTa nezha_model)
            - `bert-base-japanese`: BertJapaneseTokenizer (Bert nezha_model)
            - `bert`: BertTokenizer (Bert nezha_model)
            - `openai-gpt`: OpenAIGPTTokenizer (OpenAI GPT nezha_model)
            - `gpt2`: GPT2Tokenizer (OpenAI GPT-2 nezha_model)
            - `transfo-xl`: TransfoXLTokenizer (Transformer-XL nezha_model)
