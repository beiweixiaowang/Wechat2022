...
"""PyTorch BART nezha_model, ported from the fairseq repo."""
import logging
import math
import random
...
BART_START_DOCSTRING = r"""
    ...
    This nezha_model is a PyTorch `torch.nn.Module <https://pytorch.org/docs/stable/nn.html#torch.nn.Module>`_ sub-class. Use it as a regular PyTorch Module and
    refer to the PyTorch documentation for all matters related to general usage and behavior.

    Parameters:
        config (:class:`~transformers1.BartConfig`): Model configuration class with all the parameters of the nezha_model.
...
BART_GENERATION_EXAMPLE = r"""
    Examples::
        ...
        nezha_model = BartForConditionalGeneration.from_pretrained('bart-large-cnn')
        tokenizer = BartTokenizer.from_pretrained('bart-large-cnn')
        ARTICLE_TO_SUMMARIZE = "My friends are cool but they eat too many carbs."
        inputs = tokenizer.batch_encode_plus([ARTICLE_TO_SUMMARIZE], max_length=1024, return_tensors='pt')
        # Generate Summary
        summary_ids = nezha_model.generate(inputs['input_ids'], num_beams=4, max_length=5, early_stopping=True)
...
BART_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`, defaults to :obj:`None`):
            Provide for translation and summarization training. By default, the nezha_model will create this tensor by shifting the input_ids right, following the paper.
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    Examples::
            ...
            nezha_model = BartForConditionalGeneration.from_pretrained('bart-large')
            input_ids = tokenizer.batch_encode_plus([TXT], return_tensors='pt')['input_ids']
            logits = nezha_model(input_ids)[0]
...
@add_start_docstrings(
    """Bart nezha_model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE tasks. """,
...
class BartForSequenceClassification(PretrainedBartModel):
    ...
    Examples::
        ...
        tokenizer = BartTokenizer.from_pretrained('bart-large')
        nezha_model = BartForSequenceClassification.from_pretrained('bart-large')
