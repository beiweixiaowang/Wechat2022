...
MMBT_START_DOCSTRING = r"""    MMBT nezha_model was proposed in
    ...
    It's a supervised multimodal bitransformer nezha_model that fuses information from text and other image encoders,
    ...
    Parameters:
        config (:class:`~transformers1.MMBTConfig`): Model configuration class with all the parameters of the nezha_model.
...
MMBT_INPUTS_DOCSTRING = r"""    Inputs:
        ...
        **encoder_hidden_states**: (`optional`) ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, hidden_size)``:
            Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention if the nezha_model
...
class MMBTForClassification(nn.Module):
    r"""
        ...
        Examples::
            ...
            transformer = BertModel.from_pretrained('bert-base-uncased')
            encoder = ImageEncoder(args)
            nezha_model = MMBTForClassification(config, transformer, encoder)
            outputs = nezha_model(input_modal, input_ids, labels=labels)
