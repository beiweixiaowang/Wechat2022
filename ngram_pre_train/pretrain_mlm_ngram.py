...
from ngram_pre_train.pretrain_utils import MLM_Data, blockShuffleDataLoader
import warnings
...
def loadData(path, data_type):
    ...
    word = []
    if data_type == 'train':
        line = df['text'] + df['label'].astype('str')
        word = line.tolist()
...
test_data = loadData(path='../temp_data/test_text.pkl', data_type='unlabel')  # 1w

unlabeled_sample = random.sample(unlabeled_data, 450000)
train_all = unlabeled_sample + train_data + test_data
...
# ##
# # 加载模型
model = NeZhaForMaskedLM.from_pretrained(vocab_file_dir)
...
#     # 训练过程保存的权重
#     output_dir='./continue_pre/B_ngram_nezha',
...
#     trainer.train()
#     # 模型最终保存的权重
#     trainer.save_model('./continue_pre/B_ngram_nezha_model_6')
